...
void execute()
{
    ...
    int jmp_off = IR_meta->jmp_offset;
    ...
    switch (control->PCSource) {
        ...
        case 1:
            ...
            break;
        case 2:
            next_pipe_regs->pc = (jmp_off << 2) + ((next_pipe_regs->ALUOut >> 28) << 28);
            break;
        default:
            assert(false);
    }
...
void write_back()
{
    if (arch_state.control.RegWrite) {
        ...
        if (arch_state.control.RegDst == 0) write_reg_id = arch_state.IR_meta.reg_16_20;
...
void set_up_IR_meta(int IR, struct instr_meta *IR_meta)
{
    ...
    switch (IR_meta->opcode) {
        ...
        case ADDI:
            ...
            break;
        case LW:
            printf("Executing LW(%d), $%u = %d [%d($%u)] (function: %u) \n",
                       IR_meta->opcode,  IR_meta->reg_16_20,arch_state.registers[IR_meta->reg_16_20], IR_meta->immediate, IR_meta->reg_21_25 , IR_meta->function);
            break;
        case SW:
            printf("Executing SW(%d), $%u = $%u + %d (function: %u) \n",
                       IR_meta->opcode,  IR_meta->reg_16_20, IR_meta->reg_21_25, IR_meta->immediate, IR_meta->function);
            break;
        case BEQ:
            printf("Executing BEQ(%d) , Register $%u : %d, Register $%u : %d ? ,Branch address : %u \n", IR_meta->opcode, 
                    IR_meta->reg_16_20, arch_state.registers[IR_meta->reg_16_20],IR_meta->reg_21_25,
                    arch_state.registers[IR_meta->reg_21_25],  IR_meta -> immediate);
            break;
        case J:
            printf("Executing J(%d) , Jump Address: %u \n", IR_meta->opcode,IR_meta->jmp_offset);
            break;
...
void assign_pipeline_registers_for_the_next_cycle()
{
    ...
    if (control->PCWriteCond){
        if (curr_pipe_regs->ALUOut == 0) curr_pipe_regs->pc = next_pipe_regs->pc;
    }
